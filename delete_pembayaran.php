...
if (isset($_GET['id_paket_bimbel'])) {
    $id_paket_bimbel = $_GET['id_paket_bimbel'];
    ...
    try {
        ...
        $query_check = "SELECT id_paket_bimbel FROM pembayaran WHERE id_paket_bimbel = ?";
        ...
        $query_delete = "DELETE FROM pembayaran WHERE id_paket_bimbel = ?";
        $stmt_delete = $conn->prepare($query_delete);
        $stmt_delete->bind_param("s", $id_paket_bimbel);
        ...
        $conn->commit();
        echo "<script>alert('Data jadwal berhasil dihapus!'); window.location.href='hasil_data_pembayaran.php';</script>";
    ...
    echo "<script>alert('ID Jadwal tidak ditemukan!'); window.location.href='hasil_data_pembayaran.php';</script>";
}
?>

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_status_murid = $_POST['id_status_murid'];

    // Update the status in the database
    $query = "UPDATE master_murid SET id_status_murid = ? WHERE id_murid = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ii", $id_status_murid, $id_murid);
    $stmt->execute();
    $stmt->close();
    ...
    if ($id_murid && $id_status) {
        $stmt = $conn->prepare("UPDATE master_murid SET id_status_murid = ? WHERE id_murid = ?");
